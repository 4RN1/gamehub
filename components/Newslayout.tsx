...
// Map Georgian label → category value stored in DB
const FILTERS: { label: string; value: string }[] = [
  { label: "ყველა",   value: "all" },
  { label: "თამაშები", value: "gaming" },
   { label: "ტექი",    value: "technology" },
];

const Newslayout = ({ sectionTitle, database }: NewsLayout) => {
  const [filterByCategory, setFilterByCategory] = useState("all");

  // Apply filter: "all" → show everything, otherwise match article.category
  const filteredArticles =
    filterByCategory === "all"
      ? database
      : database?.filter((article) => article.category === filterByCategory);
          ...
          <div className="flex-1">
            ...
            <div className="flex gap-2 mb-6 flex-wrap max-sm:justify-center">
              {FILTERS.map(({ label, value }) => (
                <button
                  key={value}
                  onClick={() => setFilterByCategory(value)}
              ...
              {filteredArticles?.map((article) => (
                ...
                >
                  ...
                  <div className="relative w-85 h-55 shrink-0 overflow-hidden rounded transition">
                    <img
                      src={article.image_url ? article.image_url : "/assets/LOGO.png"}
                      alt={article.title}
                      sizes="width:100%"
